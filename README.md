...
This kit has been successfully built on the following configurations:
    ...
    PC running Linux RedHat with gcc
    PC running Linux RedHat with g++
    ...
    SUN running Solaris 2.8 with /usr/bin/cc
...
This kit has also been built on a PC running Windows98 and Windows NT
using the Cygnus port of the GNU tools
(http://sourceware.redhat.com/cygwin/.)  The versions of the Cygnus
environment were b18, b19, and b20.1.  Both gcc and g++ were used.
The CPU time measurement functions only work (in part) with b20.1.
Specifically, util_cpu_time returns the elapsed time since the
computer was booted, rather than the process's CPU time. The CPU times
nanotrav prints under b20.1 are elapsed times for the process, instead
of the usual user time for the process.  (The time measurement
functions do work under Windows NT.)  You must have bash set up
...
Directory cudd-2.3.1/cudd/doc contains HTML documentation for the CUDD
