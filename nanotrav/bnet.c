...
#ifndef lint
static char rcsid[] UTIL_UNUSED = "$Id: bnet.c,v 1.21 2001/02/15 06:59:44 fabio Exp fabio $";
#endif
...
static int bnetDumpReencodingLogic ARGS((DdManager *dd, char *mname, int noutputs, DdNode **outputs, char **inames, char **altnames, char **onames, FILE *fp));
#if 0
static int bnetBlifXnorTable ARGS((FILE *fp, int n));
#endif
static int bnetBlifWriteReencode ARGS((DdManager *dd, char *mname, char **inames, char **altnames, int *support, FILE *fp));
static int * bnetFindVectorSupport ARGS((DdManager *dd, DdNode **list, int n));
static int buildExorBDD ARGS((DdManager *dd, BnetNode *nd, st_table *hash, int params, int nodrop));
static int buildMuxBDD ARGS((DdManager * dd, BnetNode * nd, st_table * hash, int  params, int  nodrop));
...
static int bnetDfsOrder ARGS((DdManager *dd, BnetNetwork *net, BnetNode *node));
  ...
  int  pr /* verbosity level */)
{
	...
	for (i = 0; i < net->nlatches; i++) {
	    ...
	    count++;
	    if (st_lookup(net->hash, savestring, (char **) &node)) {
  ...
  int  reencoded /* whether variables have been reencoded */)
{
	...
	if (!st_lookup(network->hash,network->latches[i][0],(char **)&node)) {
	    goto endgame;
	}
	outputs[i] = node->dd;
    }
	...
	if (!st_lookup(network->hash,network->outputs[i],(char **)&node)) {
	    goto endgame;
	}
	...
	if (!st_lookup(network->hash,network->inputs[i],(char **)&node)) {
	    goto endgame;
	}
	inames[node->var] = network->inputs[i];
    }
    for (i = 0; i < network->nlatches; i++) {
	if (!st_lookup(network->hash,network->latches[i][1],(char **)&node)) {
	    goto endgame;
	}
