...
#ifndef lint
static char rcsid[] UTIL_UNUSED = "$Id: chkMterm.c,v 1.4 1998/05/09 19:12:18 fabio Exp fabio $";
#endif
...
static enum st_retval stFree ARGS((char *key, char *value, char *arg));
  ...
  st_table * previous)
{
	...
	for (i = 0; i < net->noutputs; i++) {
	    if (!st_lookup(net->hash,net->outputs[i],(char **)&po)) {
		exit(2);
	    }
	    name = net->outputs[i];
	    if (po->dd != NULL) {
	...
	for (i = 0; i < net->noutputs; i++) {
	    ...
	    name = net->outputs[i];
	    if (st_lookup(previous, name, (char **) &oldcount)) {
		if (po->dd != NULL) {
