...
#ifndef lint
static char rcsid[] UTIL_UNUSED = "$Id: ntrBddTest.c,v 1.12 2001/02/15 07:06:03 fabio Exp fabio $";
#endif
...
static int ntrTestEquivAndContainAux ARGS((DdManager *dd, BnetNetwork *net1, DdNode *f, char *fname, DdNode *g, char *gname, DdNode *d, char *dname, NtrOptions *option));
static int ntrTestClosestCubeAux ARGS((DdManager *dd, BnetNetwork *net, DdNode *f, char *fname, DdNode *g, char *gname, DdNode **vars, NtrOptions *option));
  ...
  NtrOptions *option)
{
    ...
    dsize = -1;
    for (i = 0; i < net2->noutputs; i++) {
	if (!st_lookup(net2->hash,net2->outputs[i],(char **)&node1)) {
	    return(0);
	}
    ...
    if (option->node == NULL) {
	for (i = 0; i < net1->noutputs; i++) {
	    if (!st_lookup(net1->hash,net1->outputs[i],(char **)&node1)) {
		return(0);
	    }
	    f = node1->dd;
	    if (f == NULL) return(0);
	    for (j = 0; j < net1->noutputs; j++) {
		if (!st_lookup(net1->hash,net1->outputs[j],(char **)&node2)) {
		    return(0);
		}
		g = node2->dd;
		if (g == NULL) return(0);
	...
	if (!st_lookup(net1->hash,option->node,(char **) &node1)) {
	    return(0);
	}
	f = node1->dd;
	if (f == NULL) return(0);
  ...
  NtrOptions * option)
{
    ...
    if (option->node == NULL) {
	for (i = 0; i < net->noutputs; i++) {
	    if (!st_lookup(net->hash,net->outputs[i],(char **)&node1)) {
		FREE(vars);
		return(0);
	    }
	    f = node1->dd;
		...
		if (!st_lookup(net->hash,net->outputs[j],(char **)&node2)) {
		    FREE(vars);
		    return(0);
		}
		g = node2->dd;
	...
	if (!st_lookup(net->hash,option->node,(char **) &node1)) {
	    FREE(vars);
	    return(0);
	}
	f = node1->dd;
  ...
  NtrOptions * option)
{
	...
	onames[2] = "bu";	outputs[2] = b;
	onames[3] = "hb";	outputs[3] = hb;
	onames[4] = "sp";	outputs[4] = sp;
	onames[5] = "ua";	outputs[5] = ua;
	onames[6] = "c1";	outputs[6] = c1;
	onames[7] = "c2";	outputs[7] = c2;
  ...
  NtrOptions *option)
{
	...
	(void) printf("T-X    ");
	Cudd_PrintDebug(dd, xor, nvars, pr);
	return(0);
    }
    equiv = Cudd_EquivDC(dd, f, g, Cudd_Not(d));
    if (equiv != Cudd_bddLeq(dd,xor,Cudd_Not(d))) {
	...
	onames[3] = "xor";	outputs[3] = xor;
	onames[4] = "diff";	outputs[4] = diff;
	onames[5] = "ndiff";	outputs[5] = ndiff;
	if (!Bnet_bddArrayDump(dd, net1, option->dumpfile, outputs, onames,
			       6, option->dumpFmt))
	    return(0);
    }
  ...
  NtrOptions *option)
{
	...
	onames[0] = fname;	outputs[0] = f;
	onames[1] = gname;	outputs[1] = g;
	onames[2] = "cube";	outputs[2] = cube;
	onames[3] = "cubeN";	outputs[3] = cubeN;
