...
#ifndef lint
static char rcsid[] UTIL_UNUSED = "$Id: ntr.c,v 1.22 2001/02/15 07:04:24 fabio Exp fabio $";
#endif

static char *onames[] = { "T", "R" };	/* names of functions to be dumped */
...
static DdNode * makecube ARGS((DdManager *dd, DdNode **x, int n));
static void ntrInitializeCount ARGS((BnetNetwork *net, NtrOptions *option));
static void ntrCountDFS ARGS((BnetNetwork *net, BnetNode *node));
static DdNode * ntrImage ARGS((DdManager *dd, NtrPartTR *TR, DdNode *from, NtrOptions *option));
static DdNode * ntrPreimage ARGS((DdManager *dd, NtrPartTR *T, DdNode *from, NtrOptions *option));
static DdNode * ntrChooseFrom ARGS((DdManager *dd, DdNode *neW, DdNode *reached, NtrOptions *option));
static DdNode * ntrUpdateReached ARGS((DdManager *dd, DdNode *oldreached, DdNode *to));
static int ntrLatchDependencies ARGS((DdManager *dd, DdNode *reached, BnetNetwork *net, NtrOptions *option));
static NtrPartTR * ntrEliminateDependencies ARGS((DdManager *dd, NtrPartTR *TR, DdNode **states, NtrOptions *option));
static int ntrUpdateQuantificationSchedule ARGS((DdManager *dd, NtrPartTR *T));
static int ntrSignatureCompare ARGS((int * ptrX, int * ptrY));
static int ntrSignatureCompare2 ARGS((int * ptrX, int * ptrY));
static int ntrPartCompare ARGS((int * ptrX, int * ptrY));
static char ** ntrAllocMatrix ARGS((int nrows, int ncols));
static void ntrFreeMatrix ARGS((char **matrix));
static void ntrPermuteParts ARGS((DdNode **a, DdNode **b, int *comesFrom, int *goesTo, int size));
  ...
  BnetNetwork * net2 /* companion network with which inputs may be shared */)
{
    ...
    */
	...
	for (i = 0; i < net->npis; i++) {
	    ...
	    if (!st_lookup(net2->hash,net->inputs[i],(char **) &node2)) {
	...
	for (i = 0; i < net->nlatches; i++) {
	    ...
	    if (!st_lookup(net2->hash,net->latches[i][1],(char **) &node2)) {
  ...
  NtrOptions * option /* options */)
{
	...
	dfunc[0] = TR->part[0];		onames[0] = "T";
	dfunc[1] = reached;		onames[1] = "R";
  ...
  NtrOptions * option /* program options */)
{
	...
	dfunc[1] = envelope;
	if (dumpFmt == 1) {
	    retval = Cudd_DumpBlif(dd,2,dfunc,NULL,onames,NULL,dfp);
	} else if (dumpFmt == 2) {
	    retval = Cudd_DumpDaVinci(dd,2,dfunc,NULL,onames,dfp);
	} else if (dumpFmt == 3) {
	    retval = Cudd_DumpDDcal(dd,2,dfunc,NULL,onames,dfp);
	} else if (dumpFmt == 4) {
	    retval = Cudd_DumpFactoredForm(dd,2,dfunc,NULL,onames,dfp);
	} else {
  ...
  NtrOptions * option)
{
	...
	** and build the conjuncts from the deltas. */
	...
	while (Ntr_HeapCount(T->factors) > 1) {
	    retval = Ntr_HeapExtractMin(T->factors,(void **)&factor1,&size1);
	    if (retval == 0) return(NULL);
	    retval = Ntr_HeapExtractMin(T->factors,(void **)&factor2,&size2);
	    if (retval == 0) return(NULL);
	...
	retval = Ntr_HeapExtractMin(T->factors,(void **)&image,&size1);
	if (retval == 0) return(NULL);
	Ntr_freeTR(dd,T);
    }
  ...
  NtrOptions *option)
{
    ...
    ** variables that are closer to being essential. */
    ...
    qsort((void *)candidates,net->nlatches,sizeof(int),
	  (int (*)(const void *, const void *))ntrSignatureCompare2);
#if 0
    ...
    FREE(candidates);
    if (howManySmall > 0) {
	if (!Bnet_bddArrayDump(dd,net,"-",roots,onames,howManySmall,1))
	    return(-1);
    }
  ...
  NtrOptions *option)
{
    ...
    ** variables that are closer to being essential. */
    ...
    qsort((void *)candidates,nvars,sizeof(int),
	  (int (*)(const void *, const void *))ntrSignatureCompare);
    FREE(signatures);
  ...
  NtrPartTR *T)
{
    ...
    ** in the index of the next state variable are contiguous. */
    staticPart = T->part;
    qsort((void *)position,nparts,sizeof(int),
	  (int (*)(const void *, const void *))ntrPartCompare);
  ...
  int * ptrY)
{
    BnetNode *node;
    int x,y;
    if (!st_lookup(staticNet->hash,staticNet->latches[*ptrX][1],(char **) &node)) {
	return(0);
    }
    x = node->dd->index;
    if (!st_lookup(staticNet->hash,staticNet->latches[*ptrY][1],(char **) &node)) {
	return(0);
    }
    y = node->dd->index;
