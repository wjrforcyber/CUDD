...
#ifndef lint
static char rcsid[] UTIL_UNUSED = "$Id: cuddObj.cc,v 1.3 2001/03/19 02:28:13 fabio Exp fabio $";
#endif
...
ABDD::~ABDD() {
	...
	if (ddMgr->isVerbose()) {
	    cout << "ADD/BDD destructor called for node " << hex(long(node)) <<
		" ref = " << Cudd_Regular(node)->ref << "\n";
	}
    }
...
BDD::PrioritySelect(
  ...
  BDDvector z,
  BDD Pi,
  DdNode * (*Pifunc)(DdManager * , int, DdNode **, DdNode **, DdNode **))
{
    int n = x.count();
    DdManager *mgr = ddMgr->p->manager;
    DdNode **X = ALLOC(DdNode *,n);
